...
class ExactNurV3:
    def __init__(self):
        print("Version 3 of ExactNur Algo. where pruning and termination is considered.")
                ...
                while len(S_I) < self.k <= len(S_I) + len(S_R):
                    if len(S_R) > 0:
                        ...
                        if self.FlagFS is False:
                            S_R = self.pruneE(S_I, S_R)  # Prune Lemma 4
                        else:
    ...
    def afterPrunedTList(self, S_I, S_R, bestScore):
        ...
        filteredLoc = []
        lowerLowerD_hat = self.calcLowerLowerBoundD_Hat(S_I, S_R, bestScore)
        D_Hat_Dict = self.calcDel_d_dblDash(S_I, S_R)
        FilteredPruneT = {}
        for l in D_Hat_Dict.keys():
            if D_Hat_Dict[l] > lowerLowerD_hat:
                FilteredPruneT[l] = D_Hat_Dict[l]
            #else:
                #print("location ", l, " prune T not considered")
    ...
    def calcDel_d_dblDash(self, S_I_copy, S_R_copy):
        ...
        for loc in S_R_copy:
            if len(S_I_copy) > 1:
                ...
                D_Hat_Dict[loc] = self.calcDgsOfSet(S_I_copy_temp) - min([self.dictOfDgsFromFileAndFlyNew(loc, l) for l in S_I_copy])
            else:
                S_I_copy_temp = list(S_I_copy)
                S_I_copy_temp.append(loc)
                # del_dbl_Dash_Dict[loc] = self.dictOfDgsFromFileAndFlyNew(S_I[0], loc)
                D_Hat_Dict[loc] = self.calcDgsOfSet(S_I_copy_temp) - self.dictOfDgsFromFileAndFlyNew(S_I_copy[0], loc)
