...
class SociaCoverage:
    def __init__(self):
        print("calculating social coverage when Theta = 0 and Theta = 100 KM, for FA algo")
        self.alpha = 0.5
        self.beta = 0.5
        self.omega = 0.5
        self.theta = 500
        ...
        self.createFile(os.path.join(self.outputFolder, "SC_count_k" + str(self.k) +"_FA_"+ str(self.ThetaForSC) + "KM.txt"), strContent)
        FinalResults = "Exact: ", round(finalCoLocated / len(listResultFiles), 2), "\n", "WithinTheta: ", round(finalWithinTheta / len(listResultFiles), 2)
        self.createFile(os.path.join(self.outputFolder, "FinalSC_k" + str(self.k) +"_FA_"+ str(self.ThetaForSC)+ "KM.txt"), FinalResults)
