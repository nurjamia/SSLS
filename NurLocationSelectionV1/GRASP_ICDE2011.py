...
class Grasp_GNE:
    def __init__(self):
        print("Greedy Randomized Adaptive Search Procedure GNE. ICDE 2011")
        ...
        self.k = 60  # top k items should be returned 2, 4, 6, 8, 10     20, 40, 60, 80, 100
        self.m = 10  # return top m sets
        self.imax = 10
        self.AlphaConstructVar = 0.01
        ...
        for user in self.userLocationDictUserList:
            ...
            L = self.userLocationDict[user]
            noOfLoc = len(L)            
            if user in self.socialNetwork.keys() and len(self.socialNetwork[user]) > 4 and len(set(L)) > self.k:
                print("len: self.socialNetwork[user]): ", len(self.socialNetwork[user]), "len(set(L)): ",len(set(L)))
                self.newUserList.append(user) #Will process only these users further as they have satisfied minimum requirements
        ...
        '''
        #original GRASP/ GNE starts here.
        start = time.time()
        maxRssMemory = 0
        totalRssMemory = 0
        ...
        for userId in self.newUserList:
            ...
            for i in range(0, self.imax):
                R_prime_construct = self.GNE_Construction(userId, content_div)
                R_prime_localSearch = self.GNE_LocalSearch(R_prime_construct, content_div)
                if self.calcTotalScoreofSet(R_prime_localSearch, content_div) > self.calcTotalScoreofSet(R, content_div):
                    R = list(R_prime_localSearch)
    ...
    def GNE_Construction(self, userId, content_div):
        #print("GNE Construction")
        R_p = []
        S = list(self.Loc_user_array)
        for p in range(1, self.k + 1):
            mmc_Loc_Dict = self.calc_mmc(userId, content_div, R_p, S, p)
            s_i_max_val = max(mmc_Loc_Dict.values())  # argmax 'value' from mmc_Loc_Dict
            s_i_min_val = min(mmc_Loc_Dict.values())  # argmax 'value' from mmc_Loc_Dict
            lowerVal = s_i_max_val - self.AlphaConstructVar * (s_i_max_val - s_i_min_val)
            RCL = [k for k,v in mmc_Loc_Dict.items() if v >= lowerVal]
            s_i = random.choice(RCL)
            R_p.append(s_i)
            S.remove(s_i)
        return R_p

    def GNE_LocalSearch(self, R, content_div):
        #print("GNE LocalSearch")
        S = list(self.Loc_user_array)
        R_dash = list(R)
        #print("R init: ", R)
        for s_i in R: #each element in result set (k-1) most diverse locations to be identified
            indexPos_si = -99
            try:
            ...
            diversity_all_si_list.pop(0)  # remove the fist entry of the list that contained the location s_i itself
            #heapq.nlargest(self.k - 1, diversity_all_si_list)
            #top_k_Minus_1_DivesreWith_si_Index = list(map(diversity_all_si_list.index, heapq.nlargest(self.k - 1, diversity_all_si_list))) # returns index of list with top k-1 locations with si
            index_val_tuple_list = heapq.nlargest(self.k - 1, enumerate(diversity_all_si_list), key=lambda x: x[1])

            top_k_Minus_1_DivesreWith_si_Index = []
            for item in index_val_tuple_list:
                top_k_Minus_1_DivesreWith_si_Index.append(item[0]) # get the index of the top (k-1) locations with max diversity with si

            top_k_Minus_1_DivesreWith_si = []
            for indexNum in top_k_Minus_1_DivesreWith_si_Index:
                top_k_Minus_1_DivesreWith_si.append(self.Loc_user_array[indexNum]) # get the locations of the top (k-1) locations with max diversity with si

            for s_j in R: #each of the elements in result set, swap performed
                NewSetAndTotalScoreAfterSwap = {} #contains updated R as key (in integer) and its total score
                tempDict = {} #contains, key in integer and value as updated R in list
                if s_i == s_j:
                    continue
                for l in range(1, self.k): #for l = 1 to (k-1)
                    top_si_l = top_k_Minus_1_DivesreWith_si[l - 1]
                    if top_si_l in R:
                        continue
                    R_copy = list(R)
                    R_copy.remove(s_j)
                    R_copy.append(top_si_l)
                    tempDict[l] = R_copy
                    NewSetAndTotalScoreAfterSwap[l] = self.calcTotalScoreofSet(R_copy, content_div)

                if len(NewSetAndTotalScoreAfterSwap) == 0:
                    continue
                top_R_copy_key = max(NewSetAndTotalScoreAfterSwap, key=NewSetAndTotalScoreAfterSwap.get)
                top_R_copy_val = max(NewSetAndTotalScoreAfterSwap.values())

                if top_R_copy_val > self.calcTotalScoreofSet(R_dash, content_div):
                    R_dash = list(tempDict[top_R_copy_key])
                    #print("top_R_copy_val: ",top_R_copy_val)

            if self.calcTotalScoreofSet(R_dash, content_div) > self.calcTotalScoreofSet(R, content_div):
                R = list(R_dash)
        #print("R end: ", R)
        return R
    ...
    def calcTotalScoreofSet(self, S, content_div):
        totalRel = 0
        for loc in S:
            totalRel = totalRel + float(self.loc_relScore_dict[loc])

        totalDiv = 0
        for i in range(1, len(S)): # for i=1 to k-1
            loc1 = S[i-1]
            indexPos_loc1 = -99
            try:
                indexPos_loc1 = self.Loc_user_array.index(loc1)
            except:
                print("value loc1 not available in List")
                continue
            diversity_all_loc1_list = content_div[indexPos_loc1].split("\t")
            for j in range(i+1, len(S)+1): # for j=i+1 to k
                loc2 = S[j - 1]
                indexPos_loc2 = -99
                try:
                    indexPos_loc2 = self.Loc_user_array.index(loc2)
                except:
                    print("value loc2 not available in List")
                    continue
                totalDiv = totalDiv + float(diversity_all_loc1_list[indexPos_loc2 + 1])

        TotalScore_Set = (len(S) - 1) * self.omega * totalRel + 2 * self.omega * totalDiv #scaled up the value according to the equation mentioned in paper
        return round(TotalScore_Set, 2)
