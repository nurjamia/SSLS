    ...
    def __init__(self):
        ...
        for user in self.userLocationDictUserList:
            ...
            if user in self.socialNetwork.keys() and len(self.socialNetwork[user]) > 5 and len(set(L)) > self.k:
                ...
                self.createFile(os.path.join(self.outputFolder, "Results", self.binId, "TopResultLists", str(self.k), str(user) + "_" + str(self.k) + ".txt"), self.ResultListStr)
        ...
        print("Final Time at the end:", round(time.time() - self.startBegin, 2), "k:", self.k, "bin: ", self.binId, "m: ", self.m)
