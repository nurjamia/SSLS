...
class preprocessFA:
    def __init__(self):
        print("preprocessing gowalla and brightkite to find the curve of ExactPlus using top 20% - 30% locations\n we normalize the final scores and user bin100, k = 6 as default\nUsing Location matching precision.")
        print("more clean and flexible Exact Plus version, only considers Preprocessed file...")
        ...
        self.beta = 0.5
        self.omega = 0.5
        self.k = 6  # top k items should be returned 2, 4, 6, 8, 10
        ...
        self.datasetName = "Brightkite"  # Gowalla, Brightkite, Flickr, Yelp
        ...
        self.scoreNormalized = ""
        self.divideByTopScore = ""
        self.LocCommonWithTopSet = ""
        #self.userLocationDictUserList = self.userLocationDictUserList[:50]
        for user in self.userLocationDictUserList:
            ...
            if user in self.socialNetwork.keys() and len(self.socialNetwork[user]) > 5 and len(set(L)) > self.k:
                ...
                #### Changes for preprocessing FA
                #totalLoc = len(self.L)
                #self.S_Rel = self.S_Rel[:int(totalLoc*20/100)]
                ###
                ...
                outerloop = 0
                topLocsToRun = 30 # the number of top locations to run
                while outerloop < topLocsToRun:
                    ...
                    while len(S_I) < self.k:
                        ...
                        if len(S_I) == self.k:
                            scoresList.append(self.scoreTop)
                            scoreAndLocSetDict[self.scoreTop] = S_I
                            print("added, outer loop: ", outerloop, "size: ", len(scoresList))
                            S_I = []
                            break

            if len(scoresList) > 1:
                minVal = min(scoresList)
                maxVal = max(scoresList)
                bestSet = scoreAndLocSetDict[maxVal]

                if maxVal > 0 and (maxVal - minVal) > 0:
                    for score in scoresList:
                        normalizedScore = round((score - minVal) / (maxVal - minVal), 2)
                        self.scoreNormalized = self.scoreNormalized + str(normalizedScore) + "\t"

                        common = list(set(scoreAndLocSetDict[score]) & set(bestSet))
                        commonPerc = round((len(common)/len(bestSet)), 2)
                        self.LocCommonWithTopSet = self.LocCommonWithTopSet + str(commonPerc) + "\t"

                        dividedByTop = round((score/maxVal), 2)
                        self.divideByTopScore = self.divideByTopScore + str(dividedByTop) + "\t"

                    self.scoreNormalized = self.scoreNormalized + "\n"
                    self.divideByTopScore = self.divideByTopScore + "\n"
                    self.LocCommonWithTopSet = self.LocCommonWithTopSet + "\n"


        #self.createFile(os.path.join(self.outputFolder, "Results", "NormalizedScores.txt"), self.scoreNormalized)
        #self.createFile(os.path.join(self.outputFolder, "Results", "divideByTopScore.txt"), self.divideByTopScore)
        self.createFile(os.path.join(self.outputFolder, "Results", "LocCommonWithTopSet.txt"), self.LocCommonWithTopSet)
