...
class ExactNurTSPerc:
    def __init__(self):
        print("FastApproximate Algo.")
        self.flagMinMax = "MaxSumMin"  # MaxMin, MaxSum, MaxSumMin score calculation of a set based on min max of diversity
        self.alpha = 0.5
        self.beta = 0.5
        self.omega = 0.5
        self.kPercentage = 10  # 5, 10, 15, 20 Percentage we calculate self.k based on self.kPercentage value later in the code follows: self.k = round(noOfLoc*self.kPercentage/100)
        self.m = 10  # return top m sets
        self.topSet_Score_m = []
        self.binId = "bin50"  # bin50, bin100, bin200, bin500, bin1000

        self.baseFolder = "I:\\ExpDataUDI\\ExperimentFolder"
        self.datasetName = "Brightkite"
        self.operatingFolder = os.path.join(self.baseFolder, self.datasetName)
        self.outputFolder = os.path.join(self.operatingFolder, "ExpResultFA")

        # self.twoDArray = self.loadDgsContents(os.path.join(self.outputFolder, "Diversity", str(4) + ".txt"))  # loading self.content and creating self.locAndIndex
        ...
        for user in self.userLocationDictUserList:
            ...
            if user in self.socialNetwork.keys() and len(self.socialNetwork[user]) > 5 and len(set(L)) > self.k:
                ...
                self.twoDArray = self.loadDgsContents(os.path.join(self.outputFolder, "Diversity", str(user) + ".txt"))  # loading self.content and creating self.locAndIndex
                        ...
                        if len(S_R) > 0:
                            ...
                            if len(S_I) == self.k:
                                ...
                                if len(self.topSet_Score_m) == self.m:
                                    if self.topSet_Score_m[self.m - 1][1] < score:
                                        self.topSet_Score_m.pop()
                                        self.topSet_Score_m.append((S_I, score))
                            ...
                            if len(S_I) > 1:
                                ...
                                '''
                                scoreList1 = [tupl[2] for tupl in self.Q]
                                try:
                                    indx = next(x for x, val in enumerate(scoreList1) if val > scoreS_I)
                                except:
                                    indx = len(scoreList1) - 1
                                self.Q.insert(indx, (list(S_I), list(S_R), scoreS_I))

                                scoreList2 = [tupl2[2] for tupl2 in self.Q]
                                try:
                                    indx2 = next(x for x, val in enumerate(scoreList2) if val > score_tempSI)
                                except:
                                    indx2 = len(scoreList2) - 1
                ...
                self.createFile(os.path.join(self.outputFolder, "ResultsPerc", self.binId, str(self.kPercentage), str(self.k), str(user) + "_" + str(self.k) + ".txt"), locText)
    ...
    def calcAdvTermOnTotalScore(self, S_I_copy, S_R_copy):
        ...
        totalScoreTerminateBoundLower = round((self.k - len(S_I_copy)) * (self.omega * del_s_max + (1 - self.omega) * delDash_d_max) + scoreSI, 2)
    ...
    '''
    def loadDgsContentsBkp(self, filePath):
        f = open(filePath, "r")
        self.content = f.readlines()
        self.content = [x.strip() for x in self.content]
        self.locAndIndex = []
    ...
    def caldDgsOfSet(self, SetNew):
        ...
        if self.flagMinMax == "MaxMin":
            ...
            for loc in SetNew:
                ...
                minDgs = min([self.dictOfDgsFromFileAndFlyNewLatest(loc, l) for l in S_setminus])
        ...
        if self.flagMinMax == "MaxSum":
            ...
            for loc in SetNew:
                S_setminus = set(SetNew)
                S_setminus.remove(loc)
                minDgs = sum([self.dictOfDgsFromFileAndFlyNewLatest(loc, l) for l in S_setminus])
                totalDgs += minDgs
            return totalDgs
    ...
    def calcDel_d_dblDash(self, S_I_copy, S_R_copy):
        ...
        for loc in S_R_copy:
            if len(S_I_copy) > 1:
                ...
                del_d_dbl_Dash_Dict[loc] = self.calcTotalDgsOfSet(S_I_copy_temp) - min([self.dictOfDgsFromFileAndFlyNewLatest(loc, l) for l in S_I_copy])
            else:
                S_I_copy_temp = list(S_I_copy)
                S_I_copy_temp.append(loc)
                # del_dbl_Dash_Dict[loc] = self.dictOfDgsFromFileAndFlyNew(S_I[0], loc)
                del_d_dbl_Dash_Dict[loc] = self.calcTotalDgsOfSet(S_I_copy_temp) - self.dictOfDgsFromFileAndFlyNewLatest(S_I_copy[0], loc)
        ...
        if self.flagMinMax == "MaxSum":
            ...
            for loc in S:
                ...
                for loc2 in S_setminus:
                    ...
                    totalDgs += self.dictOfDgsFromFileAndFlyNewLatest(loc, loc2)
