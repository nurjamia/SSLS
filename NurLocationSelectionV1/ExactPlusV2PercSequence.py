...
class ExactPlusV2Perc:
    def __init__(self):
        ...
        #checkBinsList = ["bin50", "bin100", "bin200", "bin500", "bin1000"]
        kPercentageList = [5, 15, 20]

        for kPercentage in kPercentageList:
            #self.binId = binId
            self.kPercentage = kPercentage
            ...
            for user in self.userLocationDictUserList:
                ...
                noOfLoc = len(L)
                if self.datasetName == "Flickr" or self.datasetName == "Yelp":
                    self.k = round(len(set(L)) * self.kPercentage / 100)
                else:
                ...
                if user in self.socialNetwork.keys() and len(self.socialNetwork[user]) > 5 and len(set(L)) > self.k:
                    ...
                    print("processing user:", user, ",self.kPercentage: ", self.kPercentage, ", self.binId: ", self.binId,  ", self.k:", self.k, ", friends:", len(ngbrsList), ", ngbr number of loc:", len(self.ngbrLocCombinedUnique), ", noOfLoc: ", noOfLoc, ", uniQ Loc: ", len(set(L)))
