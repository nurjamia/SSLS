...
class MMR:
    def __init__(self):
        print("Greedy based Maximum Marginal Relevance (MMR) as baseline. ICDE 2011")
        ...
        for user in self.userLocationDictUserList:
            ...
            if user in self.socialNetwork.keys() and len(self.socialNetwork[user]) > 5 and len(set(L)) > self.k:
                ...
                if len(self.ngbrLocCombinedUnique) < 1:
                    continue
                self.L = list(set(L))  # converting into set and then in list. It will contain unique elements
        ...
        for userId in self.newUserList:
            ...
            if len(R) == 0:
                topRelLoc = max(self.loc_relScore_dict, key = self.loc_relScore_dict.get)
                R.append(topRelLoc)
                S.remove(topRelLoc)
            while len(R) < self.k:
                mmr_Loc_Dict = self.calc_mmr(content_div, R, S)
                topLoc = max(mmr_Loc_Dict, key = mmr_Loc_Dict.get)
                R.append(topLoc)
                S.remove(topLoc)
    ...
    def calc_mmr(self, content_div, R, S):
        #print("MMR calculation")
        mmr_dict = {}
        for s_i in S:
            ...
            mmr_score = round(self.omega * rel_s_i + ((1 - self.omega) / len(R)) * sum(del_div_val) , 2)
            mmr_dict[s_i] = mmr_score
        return mmr_dict
