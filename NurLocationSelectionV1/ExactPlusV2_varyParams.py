...
class ExactPlusV2:
    def __init__(self):
        print("more clean and flexible Exact Plus version, only considers Preprocessed file vary parameters alpha, beta, omega...")
        ...
        for user in self.userLocationDictUserList:
            ...
            if user in self.socialNetwork.keys() and len(self.socialNetwork[user]) > 5 and len(set(L)) > self.k:
                ...
                self.createFile(os.path.join(self.outputFolder, "Results",  self.binId, str(self.k), "beta", str(self.param), str(user) + "_" + str(self.k) + ".txt"), locText)
        ...
        self.createFile(os.path.join(self.outputFolder, "Results", self.binId, str(self.k), "beta", str(self.param),"TimeAndStats.txt"), strTimeStat)
